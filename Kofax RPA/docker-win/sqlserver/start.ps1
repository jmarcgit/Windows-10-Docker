...
###############  Modification of original entrypoint BEGIN
if(![System.IO.File]::Exists("C:\SQLData\scheduler.mdf")) {   # create shcheduler db if not existed
    & sqlcmd -i c:\script.sql
#    echo $null >> c:\sheduler_db.lck
}
###############  Modification of original entrypoint END
